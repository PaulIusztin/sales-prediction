...
import data
from datasets import MonthPriceSalesDataset
...
class LightGBMTrainer:
    ...
    def load_data(self) -> Dict[str, Tuple[pd.DataFrame, pd.DataFrame]]:
        df = data.try_load_from_cache(features=self.FEATURES)
        if df is None:
            df = data.load()
            df = data.pre_clean(df)
            df = data.add_features(df=df, features=self.FEATURES)
            data.save_to_cache(df, features=self.FEATURES)
...
class MonthlyLightGBMTrainer:
    ...
    def __init__(self, dataset: MonthPriceSalesDataset):
        self.experiment_name = f"lightgbm_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
        self.output_folder = Path("../outputs") / self.experiment_name
        self.output_folder.mkdir(parents=True, exist_ok=True)
        print(f"Output folder: {self.output_folder.absolute()}")
...
if __name__ == "__main__":
    ...
    trainer = MonthlyLightGBMTrainer(dataset=dataset)
    trainer.train()
    trainer.test()
